...
/*   Updated: 2024/07/07 22:37:18 by vkhrabro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_square(t_data *data, int x, int y, int color) {
    for (int j = y; j < y + data->minimap_cell_size; j++) {
        for (int i = x; i < x + data->minimap_cell_size; i++) {
...
void draw_player(t_data *data, int x, int y, int size) {
    ...
    draw_square(data, x - data->minimap_cell_size / 2, y - data->minimap_cell_size / 2, PLAYER_COLOR);
}
...
void put_texture_pixel(t_data *data, int x, int y)
{
	...
	{
        char *src = data->texture.addr + (data->tex_y * data->texture.line_length + data->tex_x * (data->texture.bits_per_pixel / 8));
        char *dst = data->addr + (y * data->line_length + x * (data->bits_per_pixel / 8));
...
void render_map(t_data *data)
{
    ...
    data->minimap_y = WINDOW_HEIGHT - (data->map.height * data->minimap_cell_size) - 20; // Position from the bottom
	data->i = 0;
        ...
        while (data->j < (int)data->map.width) 
		{
            if (data->j < (int)ft_strlen(data->map.array[data->i]))
			{
                if (data->map.array[data->i][data->j] == '1'){
                    color = WALL_COLOR;
                    draw_square(data, data->minimap_x + data->j * data->minimap_cell_size, data->minimap_y + data->i * data->minimap_cell_size, color);
                }
                else if (data->map.array[data->i][data->j] == '0'){
                    color = EMPTY_COLOR;
                    draw_square(data, data->minimap_x + data->j * data->minimap_cell_size, data->minimap_y + data->i * data->minimap_cell_size, color);
                }
				else if (data->map.array[data->i][data->j] == data->map.player_orientation)
					init_player(data);
            }
			data->j++;
        }
...
void render_3d_view(t_data *data)
{
	...
	// calculating the height and position of the wall segment to be drawn on the screen after a ray has hit a wall
        if (hit == 1)
		{
            ...
            data->tex_x = (int)(wall_x * (double)data->texture.width);
            if (side == 0 && ray_x > 0) data->tex_x = data->texture.width - data->tex_x - 1;
            if (side == 1 && ray_y < 0) data->tex_x = data->texture.width - data->tex_x - 1;

            double step = 1.0 * data->texture.height / line_height;
